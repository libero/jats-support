...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt">
    ...
    <pattern id="article-type">
        <rule context="@article-type">
            ...
            <assert id="article-type-assert-2" test="count(parent::article[front/article-meta/article-categories/subj-group[@subj-group-type='heading']/subject])=0" role="warn">
                @<name/> is ignored if there is a &lt;subj-group subj-group-type="heading"&gt;.
            </assert>
        </rule>
    </pattern>
    ...
    <pattern id="year_parent">
        <rule context="year[parent::*]">
            <let name="parent" value="name(..)"/>
            <assert id="year_parent-assert-1" test="$parent='pub-date' or $parent='element-citation'" role="warn">
                &lt;<name/>&gt; in &lt;<value-of select="$parent"/>&gt; is ignored.
            </assert>
        </rule>
    </pattern>
